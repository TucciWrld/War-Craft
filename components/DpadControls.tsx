...
const FireButton: React.FC = () => {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    // Initial fire
    window.dispatchEvent(new CustomEvent('fire-lasers'));

    // Continuous auto-fire interval
  ...
  return (
    <div
      ...
      className={`w-14 h-14 sm:w-20 sm:h-20 bg-red-600/30 backdrop-blur-sm border border-red-500/50 rounded-full flex flex-col items-center justify-center text-white transition-all duration-100 ease-in-out select-none touch-none shadow-lg shadow-red-500/10 ${isPointerPressed ? 'bg-red-500/60 scale-90 shadow-red-500/30' : 'animate-pulse'}`}
      onPointerDown={handlePointerDown}
      onPointerUp={handlePointerUpOrCancel}
      onPointerCancel={handlePointerUpOrCancel}
      onLostPointerCapture={handlePointerUpOrCancel}
    >
      <span className="text-[10px] sm:text-xs font-black tracking-widest text-red-300 relative z-10 -mb-0.5 font-sans">FIRE</span>
      <svg className="w-5 h-5 sm:w-7 sm:h-7 text-red-400 relative z-10 animate-[spin_6s_linear_infinite]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
